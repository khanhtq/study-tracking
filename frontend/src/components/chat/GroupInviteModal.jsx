        ...
        >
          ...
          <div className="flex items-center justify-between px-5 py-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/40">
            <div className="flex items-center gap-2.5">
              ...
              <h3 className="text-sm font-bold text-slate-900 dark:text-slate-100">{t('invite_link')}</h3>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="p-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/20 space-y-3">
            <div className="text-xs font-semibold text-slate-700 dark:text-slate-300">{t('create_invite_link')}</div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-[11px] text-slate-500 dark:text-slate-400 mb-1 block">Thời hạn</label>
                <select
                  value={expiresInDays}
                  onChange={(e) => setExpiresInDays(Number(e.target.value))}
                  className="w-full bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl px-3 py-2 text-xs text-slate-900 dark:text-slate-200 focus:outline-none focus:border-indigo-500"
                >
              ...
              <div>
                <label className="text-[11px] text-slate-500 dark:text-slate-400 mb-1 block">Số lượt dùng tối đa</label>
                <select
                  value={maxUses}
            ...
            <button
              ...
              disabled={creating}
              className="w-full flex items-center justify-center gap-1.5 py-2 px-3 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold transition-all shadow-md shadow-indigo-600/20 disabled:opacity-50 cursor-pointer"
            >
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-2.5 min-h-[200px]">
            <div className="text-xs font-semibold text-slate-500 dark:text-slate-400 mb-2">Link mời đang hoạt động</div>
            {loading ? (
                  ...
                  >
                    <div className="space-y-1 overflow-hidden pr-2">
                      <div className="flex items-center gap-2">
                        <span className="font-mono text-xs font-bold text-indigo-600 dark:text-indigo-400 tracking-wider">{invite.code}</span>
                        {invite.expiresAt && (
                          <span className="flex items-center gap-1 text-[10px] text-slate-600 dark:text-slate-400 bg-slate-200 dark:bg-slate-800/80 px-2 py-0.5 rounded-md">
                      ...
                      <div className="text-[11px] text-slate-500 dark:text-slate-400 truncate">
                        Đã dùng: <span className="text-slate-900 dark:text-slate-200 font-semibold">{invite.usedCount}</span> / {invite.maxUses || '∞'}
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-1.5 flex-shrink-0">
                      <button
                        ...
                        className={`flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all cursor-pointer shadow-xs ${
                          isCopied
                            ? 'bg-emerald-600 text-white'
                            : 'bg-indigo-50 dark:bg-indigo-600/20 text-indigo-600 dark:text-indigo-300 hover:bg-indigo-600 hover:text-white border border-indigo-200 dark:border-indigo-500/30'
                        }`}
                      >
