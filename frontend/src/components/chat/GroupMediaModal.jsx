  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
      ...
      <motion.div
        ...
        className="w-full max-w-4xl bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl overflow-hidden flex flex-col h-[85vh] max-h-[750px]"
      >
        {/* Header */}
        ...
        {/* Toolbar: Tabs & Search */}
        <div className="p-4 border-b border-slate-800/80 bg-slate-950/20 flex flex-col sm:flex-row gap-3 items-center justify-between">
          {/* Tabs */}
          ...
          <div className="relative w-full sm:w-64">
            ...
            <input
              ...
              className="w-full bg-slate-950 border border-slate-800 focus:border-indigo-500/50 rounded-xl pl-9 pr-3 py-1.5 text-xs text-slate-200 placeholder-slate-500 focus:outline-none transition-colors"
            />
          </div>
        </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 sm:gap-4">
              {filteredAttachments.map((att) => {
                ...
                return (
                  <div
                    key={att.id}
                    className="group relative flex flex-col bg-slate-950/60 border border-slate-800/80 hover:border-indigo-500/40 rounded-2xl overflow-hidden transition-all shadow-md hover:shadow-indigo-500/5"
                  >
                    {/* Media Preview Box */}
                    <div
                      className="relative w-full aspect-video sm:aspect-square bg-slate-900 flex items-center justify-center overflow-hidden cursor-pointer"
                      onClick={() => {
                        if (isImage) {
                          setPreviewImage(att);
                        } else {
                    ...
                    >
                      ...
                      <div className="absolute inset-0 bg-slate-950/70 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2 p-2 backdrop-blur-xs">
                        ...
                        <button
                          ...
                          disabled={isSaving}
                          className="p-2 rounded-xl bg-slate-800/80 hover:bg-emerald-600 text-white transition-colors disabled:opacity-50"
                    ...
                    <div className="p-2.5 flex flex-col justify-between flex-1 gap-1.5">
                      <p className="text-xs font-bold text-slate-200 truncate" title={att.fileName}>
                        {att.fileName}
                      </p>
                      ...
                      <div className="pt-1.5 border-t border-slate-800/60 flex items-center justify-between text-[10px] text-slate-400">
                        <span className="truncate max-w-[100px]" title={att.senderName}>
                          bởi <strong className="text-slate-300 font-semibold">{att.senderName}</strong>
                        </span>
                        <button
                          onClick={(e) => handleSaveToDrive(e, att)}
                          disabled={isSaving}
