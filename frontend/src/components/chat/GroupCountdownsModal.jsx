      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            className="relative w-full max-w-lg h-[560px] max-h-[85vh] glass-panel rounded-3xl border border-slate-800 bg-slate-900/95 shadow-2xl overflow-hidden flex flex-col z-10 transform-gpu"
          >
            {/* Header */}
            ...
            <div className="p-5 overflow-y-auto space-y-4 flex-1">
              {loading ? (
                <div className="py-12 flex flex-col items-center justify-center gap-3 text-slate-400">
                  ...
                  <span className="text-xs font-medium">{t('loading_countdowns')}</span>
                </div>
              ) : countdowns.length === 0 ? (
                  ...
                  {countdowns.map((item, idx) => {
                    ...
                    return (
                      <div
                        ...
                        className="p-4 rounded-2xl bg-slate-950/70 hover:bg-slate-950/90 border border-slate-800 hover:border-slate-700 transition-all flex items-center justify-between gap-3 group"
                      >
                ...
                <div className="pt-2 border-t border-slate-800">
                  {!showAddSection ? (
                    <button
                      ...
                      className="w-full py-2.5 px-4 rounded-2xl bg-indigo-500/10 hover:bg-indigo-500/20 border border-indigo-500/30 text-indigo-400 text-xs font-bold flex items-center justify-center gap-2 transition-all cursor-pointer shadow-sm"
                    >
                    ...
                    <div className="p-4 rounded-2xl bg-slate-950/80 border border-slate-800 space-y-3 animate-fadeIn">
                      ...
                      {unlinkedAvailable.length === 0 ? (
                        <div className="text-xs text-slate-400 py-2 text-center font-medium">
                        ...
                        <select
                          ...
                          className="w-full px-3 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-xs text-slate-100 font-medium focus:outline-none focus:border-indigo-500 cursor-pointer shadow-sm"
                        >
                          <option value="" className="bg-slate-900 text-slate-400">{t('select_event_to_link')}</option>
                          {unlinkedAvailable
                            ...
                            .map((ev, idx) => {
                              ...
                              return (
                                <option key={key || `ev-${idx}`} value={key} className="bg-slate-900 text-slate-100">
                      ...
                      {unlinkedAvailable.length > 0 && (
                        <button
                          ...
                          className="w-full py-2.5 px-4 rounded-xl bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white font-bold text-xs flex items-center justify-center gap-2 shadow-md shadow-indigo-600/20 transition-all cursor-pointer"
                        >
                          {actionLoading ? (
            ...
            <div className="p-4 border-t border-slate-800 bg-slate-900/60 flex justify-end">
              <button
                onClick={onClose}
                className="px-4 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 hover:text-white border border-slate-700 text-xs font-semibold transition-colors cursor-pointer"
              >
                {t('btn_close')}
              </button>
            </div>
