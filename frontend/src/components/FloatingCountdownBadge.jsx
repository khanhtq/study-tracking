...
export default function FloatingCountdownBadge({ activeCountdown, onClick }) {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
      <button
        onClick={onClick}
        className="group bg-slate-900/90 hover:bg-slate-900 backdrop-blur-md border border-indigo-500/40 hover:border-indigo-400 text-slate-100 shadow-2xl shadow-indigo-950/60 px-4 py-2.5 rounded-full transition-all duration-300 flex items-center gap-2.5 cursor-pointer hover:scale-[1.03]"
      ...
      >
        ...
        <div className="bg-indigo-600/30 border border-indigo-500/40 text-indigo-300 font-mono font-bold text-xs px-2 py-0.5 rounded-full">
          {daysRemaining} {t('countdown_days')}
        </div>

        <ChevronUp className="w-3.5 h-3.5 text-slate-400 group-hover:text-slate-200 transition-transform group-hover:-translate-y-0.5" />
      </button>
    </div>
  );
