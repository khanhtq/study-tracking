    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-md">
        <motion.div
          ...
          className="w-full max-w-2xl h-[600px] max-h-[90vh] glass-panel rounded-3xl border border-slate-800 bg-slate-900/95 shadow-2xl overflow-hidden flex flex-col transform-gpu"
        >
          {/* Modal Header */}
          ...
          {/* Navigation Tabs */}
          <div className="flex border-b border-slate-800 px-6 bg-slate-950/40 gap-6">
            <button
              onClick={() => { setActiveTab('presets'); setIsFormOpen(false); }}
              className={`py-3.5 text-xs font-bold transition-all relative border-b-2 cursor-pointer flex items-center gap-2 ${
                activeTab === 'presets'
            ...
            >
              ...
              <span>{t('countdown_tab_presets')}</span>
            </button>
            <button
            ...
            >
              ...
              <span>{t('countdown_my_events')} ({activeEvents.length})</span>
            </button>
          </div>
            ...
            {activeTab === 'presets' && (
              <div className="space-y-4">
                  ...
                  <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                    ...
                    <div className="relative flex-1">
                      ...
                      <input
                        ...
                        className="w-full pl-9 pr-8 py-2 bg-slate-950/80 border border-slate-800 rounded-xl text-xs text-slate-100 placeholder:text-slate-500 focus:outline-none focus:border-indigo-500/50 transition-all font-medium"
                      />
                      {searchQuery && (
                        <button
                    ...
                    <div className="flex items-center gap-1 bg-slate-950/60 p-1 border border-slate-800 rounded-xl shrink-0">
                      <button
                        ...
                        className={`px-3 py-1 text-xs rounded-lg transition-all cursor-pointer ${
                          presetFilter === 'all'
                            ? 'bg-slate-800 text-slate-100 font-bold shadow-sm border border-slate-700/60'
                ...
                {filteredPresets.length === 0 ? (
                  <p className="text-center text-slate-400 text-xs py-8 font-medium">Không tìm thấy kỳ thi hoặc sự kiện nào phù hợp.</p>
                ) : (
                  filteredPresets.map((preset) => {
                    ...
                    return (
                      <div
                        key={preset.examCode}
                        className={`p-4 rounded-2xl border flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 transition-all group ${
                          isPinned
                            ? 'bg-indigo-500/5 border-indigo-500/30 shadow-sm'
                            : isTracking
                            ? 'bg-slate-950/90 border-slate-800'
                            : 'bg-slate-950/70 hover:bg-slate-950/90 border-slate-800 hover:border-slate-700'
                        }`}
                      >
                        <div className="min-w-0 flex-1 space-y-2">
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {isPinned && (
                              <span className="text-[10px] bg-amber-500/10 text-amber-400 border border-amber-500/20 px-2 py-0.5 rounded-full font-bold inline-flex items-center gap-1">
                          ...
                          <div className="flex items-center gap-1.5 flex-wrap">
                            {isTracking && !isPinned && (
                              <span className="text-[10px] bg-emerald-500/10 text-emerald-400 font-bold px-2 py-0.5 rounded-md border border-emerald-500/20 whitespace-nowrap">
                                {t('countdown_btn_tracking')}
                              </span>
                            )}
                            {preset.isOfficialDate ? (
                              <span className="text-[10px] bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 px-2 py-0.5 rounded-md font-bold inline-flex items-center gap-1 whitespace-nowrap">
                                <ShieldCheck className="w-3 h-3 text-emerald-400" /> {t('countdown_official_badge')}
                              </span>
                            ) : preset.isCommunityEvent ? (
                              <span className="text-[10px] bg-purple-500/10 text-purple-400 border border-purple-500/20 px-2 py-0.5 rounded-md font-bold inline-flex items-center gap-1 whitespace-nowrap">
                                <Globe className="w-3 h-3 text-purple-400" />
                                {t('countdown_created_by')} {preset.creatorDisplayName ? `@${preset.creatorDisplayName}` : 'Thành viên'}
                              </span>
                            ) : (
                              <span className="text-[10px] bg-slate-800 text-slate-300 border border-slate-700 px-2 py-0.5 rounded-md font-bold whitespace-nowrap">
                                {t('countdown_estimated_badge')}
                              </span>
                            )}

                            <span className="text-[10px] bg-slate-800 text-slate-400 border border-slate-700 px-2 py-0.5 rounded-md font-bold inline-flex items-center gap-1 whitespace-nowrap">
                              <Users className="w-3 h-3 text-slate-400" />
                              {displayTrackerCount} {t('countdown_trackers_count')}
                            </span>
                          </div>

                          {examDesc && <p className="text-xs text-slate-400 line-clamp-2 leading-relaxed">{examDesc}</p>}
                          ...
                          {isTracking ? (
                            <>
                              {!isPinned && (
                                <button
                                  ...
                                  className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 hover:text-white border border-slate-700 rounded-xl text-xs font-semibold transition-all flex items-center gap-1.5 cursor-pointer shadow-sm"
                              ...
                              <button
                                ...
                                className="px-3 py-1.5 bg-emerald-500/10 hover:bg-rose-500/10 text-emerald-400 hover:text-rose-400 border border-emerald-500/20 hover:border-rose-500/30 rounded-xl text-xs font-semibold flex items-center gap-1.5 group/untrack transition-all cursor-pointer"
                            ...
                            <div className="flex items-center gap-1.5">
                              <button
                                ...
                                className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 hover:text-white border border-slate-700 rounded-xl text-xs font-semibold transition-all cursor-pointer"
                              >
                              ...
                              <button
                                ...
                                className="px-3 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 shadow-md shadow-indigo-600/20 cursor-pointer"
                                title={t('countdown_pin_tooltip')}
                              >
                ...
                {!isFormOpen ? (
                  <>
                    <button
                      ...
                      className="w-full py-3 bg-slate-950/40 hover:bg-slate-800/60 text-indigo-400 border border-dashed border-slate-800 hover:border-indigo-500/50 rounded-2xl text-xs font-bold transition-all flex items-center justify-center gap-2 cursor-pointer shadow-sm"
                    >
                      <Plus className="w-4 h-4 text-indigo-400" /> {t('countdown_add_new')}
                    </button>

                    {activeEvents.length === 0 ? (
                      <div className="text-center py-10 space-y-3">
                        <Calendar className="w-10 h-10 text-slate-600 mx-auto" />
                        <p className="text-slate-400 text-xs font-medium">{t('countdown_empty')}</p>
                        <button
                          onClick={() => setActiveTab('presets')}
                          className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-xs font-bold transition-all inline-flex items-center gap-2 cursor-pointer shadow-md shadow-indigo-600/20"
                        >
                              ...
                              <div className="min-w-0 flex-1 space-y-1.5">
                                ...
                                <div className="flex items-center gap-2 text-xs text-slate-400 flex-wrap">
                                  ...
                                  {isPreset ? (
                                    ...
                                    <span className="text-[10px] bg-purple-500/10 text-purple-400 px-1.5 py-0.5 rounded-md border border-purple-500/20 font-bold">
                  ...
                  <form onSubmit={handleSubmitCustom} className="space-y-4 bg-slate-950/60 border border-slate-800 p-5 rounded-2xl">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3.5 py-2.5 text-xs text-slate-100 placeholder:text-slate-500 focus:outline-none focus:border-indigo-500 transition-colors shadow-sm font-medium"
                      />
                    </div>
                    ...
                    <div className="space-y-2.5 pt-2 border-t border-slate-800">
                      <div className="flex items-center gap-2.5">
                        ...
                        />
                        <label htmlFor="isPinned" className="text-xs text-slate-200 font-bold cursor-pointer flex items-center gap-1.5">
                      ...
                      <div className="flex items-center gap-2.5">
                        ...
                        />
                        <label htmlFor="isCommunityEvent" className="text-xs text-slate-200 font-bold cursor-pointer flex items-center gap-1.5">
                      ...
                      <div className="flex items-center gap-2.5">
                        <input
                          ...
                          className="w-4 h-4 rounded border-slate-700 bg-slate-950 text-indigo-600 focus:ring-indigo-500 cursor-pointer"
                        />
                        <label htmlFor="emailNotify" className="text-xs text-slate-300 font-medium cursor-pointer">
                        ...
                        <div className="flex items-center justify-between gap-2 pt-3 border-t border-slate-800 flex-wrap">
                          {editingEventId ? (
                            <div className="flex flex-col gap-1">
                              ...
                              {!canDeleteEvent && (
                                <span className="text-[11px] text-amber-400/90 italic font-semibold">
                          ...
                          <div className="flex items-center gap-2 ml-auto">
                            <button
                              type="button"
                              onClick={() => { setIsFormOpen(false); setEditingEventId(null); }}
                              className="px-3.5 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 hover:text-white border border-slate-700 rounded-xl text-xs font-semibold transition-colors cursor-pointer"
                            >
                            ...
                            <button
                              type="submit"
                              disabled={submitting}
                              className="px-4 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-xs font-bold transition-colors cursor-pointer shadow-md shadow-indigo-600/20"
                            >
