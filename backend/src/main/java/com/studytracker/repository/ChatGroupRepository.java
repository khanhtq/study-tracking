...
public interface ChatGroupRepository extends JpaRepository<ChatGroup, UUID> {
    ...
    List<ChatGroup> findByOwnerIdAndIsArchivedFalseOrderByCreatedAtDesc(UUID ownerId);

    @Query("SELECT g FROM ChatGroup g WHERE g.privacy = 'PUBLIC' AND g.isArchived = false ORDER BY g.popularityScore DESC, g.memberCount DESC")
